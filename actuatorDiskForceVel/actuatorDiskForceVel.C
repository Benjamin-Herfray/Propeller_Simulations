...
#include "actuatorDiskForceVel.H"
...
namespace fv
{
	defineTypeNameAndDebug(actuatorDiskForceVel, 0); // Debugging
	addToRunTimeSelectionTable(option, actuatorDiskForceVel, dictionary);
}
}


Foam::fv::actuatorDiskForceVel::actuatorDiskForceVel
(
	const word& name,
...
:
	...
	V_(coeffs_.get<vector>("freestreamVel"))
{
...
void Foam::fv::actuatorDiskForceVel::addSup // Add volume force to equations of motion
(
	fvMatrix<vector>& eqn,
    const label fieldi
)
{
	...
	const vectorField& U = eqn.psi(); // Get value of fluid velocity to be used for force calculations

	calcActuatorDiskVolForce(interiorRadius_, exteriorRadius_, startPoint_, endPoint_, Ct_, Cp_, rpm_, V_, U, volumeForce); // Calculate the volume force Field
...
bool Foam::fv::actuatorDiskForceVel::read(const dictionary& dict) // Not sure, but seems to be important. Best guess is that this ensures routine is not read if class is not implemented in fvOptions
{
	NotImplemented;
...
scalar Foam::fv::actuatorDiskForceVel::calcAxialForce // Computes force in axial direction
(
	const scalar& r,
...
{ 
	...
	return rs * sqrt(1.0 - rs) * (1 - W / (r * rpm_ * constant::mathematical::pi / 30)); // Fixed circulation with force determined by velocity. Will be normalized later.
}

scalar Foam::fv::actuatorDiskForceVel::calcTanForce // Tangential force
(
	const scalar& r,
	const scalar& interiorRadius, 
	const scalar& exteriorRadius, 
...
)
{
	...
	return rs * sqrt(1.0 - rs) / r * U; // Fixed circulation with force determined by velocity. Will be normalized later.
}

void Foam::fv::actuatorDiskForceVel::calcActuatorDiskVolForce // Calculate the volume force vector field applied to disk region
(
	//const fvMesh& iMesh,
	const scalar& interiorRadius,
	const scalar& exteriorRadius,
...
) 
{
	...
	scalar totalThrust = 0; // Keep track of total thrust and torque applied to cells
	scalar totalTorque = 0;

	scalar Vn = 0; // Freestream velocity
	scalar Un = 0; // Velocity normal to the propeller
	scalar W = 0; // Velocity in tangential direction
	...
	forAll(cells_, i) 
	{
		...
		if(mag(tangentialDirection) != 0) {
			tangentialDirection /= mag(tangentialDirection); // Normalize if the point is not on the central axis. If it is, we take the tangential force to be 0 which can be achieved by keeping the tangential vector = 0.		
		}

		Vn = vecAxial & V; // Freestream velocity
		Un = U[celli] & vecAxial; // Velocity normal to the propeller
		W = U[celli] & tangentialDirection; // Velocity in tangential direction	
		
		totalThrust += calcAxialForce(radialDist, interiorRadius, exteriorRadius, startPoint, endPoint, T, Vn, Un, W) * mesh_.V()[celli]; // Calculate total thrust
		totalTorque += radialDist * calcTanForce(radialDist, interiorRadius, exteriorRadius, startPoint, endPoint, Q, Vn, Un) * mesh_.V()[celli]; // Calculate total torque
	}

	forAll(cells_, i) // Repeat loop, but this time adding to the force field with expressions normalized by totalThrust and totalTorque to ensure accurate total values
	{
		const label celli = cells_[i]; // New label for the i'th cell
		...
		volumeForce[celli] = vecAxial * calcAxialForce(radialDist, interiorRadius, exteriorRadius, startPoint, endPoint, T, Vn, Un, W) * T / totalThrust / 4; // Ensures total thrust is T / 4. The factor of 4 comes from the fact that we are simulating over a quarter domain
		if (totalTorque != 0) {
			volumeForce[celli] += tangentialDirection * calcTanForce(radialDist, interiorRadius, exteriorRadius, startPoint, endPoint, Q, Vn, Un) * Q / totalTorque / 4; // Ensures total torque is Q / 4
		}
	}
	
}

void Foam::fv::actuatorDiskForceVel::writeVTK() // Write the outer surface of the actuator disk to a VTK file so that it can be visualized in Paraview.
{ 
